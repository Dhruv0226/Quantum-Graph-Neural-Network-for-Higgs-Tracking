...
[![PyTorch Geometric](https://img.shields.io/badge/PyG-Graph%20Learning-3C2179)](https://pytorch-geometric.readthedocs.io/)
...
| **3. Data generation** | `CERNOpenDataDownloader` pulls public datasets — Higgs→4ℓ, Higgs→ττ, CMS tracks — from the [CERN Open Data Portal](http://opendata.cern.ch/) |
...
1. Open `HiggsTracking (4).ipynb` in [Google Colab](https://colab.research.google.com/).
2. Run the cells in order — the first will mount your Google Drive (accept the auth prompt).
3. The dependency cell installs `torch-geometric` and `uproot`.
4. Data downloads automatically from the CERN Open Data Portal on first run and is cached for subsequent runs.
5. GPU vs. CPU is auto-detected, adjusting batch size, epochs, and hidden dimensions accordingly — so it runs on CPU too, just with fewer events.
...
This project was built collaboratively as part of the original [kaustubh2204/Quantum-Graph-Neural-Network-for-Higgs-Tracking](https://github.com/kaustubh2204/Quantum-Graph-Neural-Network-for-Higgs-Tracking) repository.
